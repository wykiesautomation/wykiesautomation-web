...
(function(){
  ...
  // Checkout (PayFast)
  window.checkout = async function(sku){
    try{
      const cfg = await fetch('config.json').then(r=>r.json());
      const res = await fetch(cfg.appsScriptUrl+'?action=create_checkout&sku='+encodeURIComponent(sku));
      const data = await res.json();
      if(data && data.redirectUrl){ location.href = data.redirectUrl; }
      else{ alert(data.message||'Checkout not configured'); }
    }catch(e){ alert('Checkout unavailable (offline).'); }
  };
    ...
    div.innerHTML = `
      ...
      <div class="content">
        ...
        <div class="actions">
          ...
          <a class="btn primary" href="#" onclick="checkout('${p.sku}')">Checkout</a>
        </div>
      </div>`;
    return div;
  }
  ...
  // Render immediate fallbacks (file:// safe)
  ...
  const status = document.getElementById('status'); if(status) status.style.display='block';
  renderAll();

  // Try to load live data (replace fallbacks if available)
