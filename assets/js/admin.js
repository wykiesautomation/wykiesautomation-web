...
async function importXlsx(){
  const fileId = document.getElementById('xlsxFileId').value.trim();
  if(!fileId){ alert('Paste a Google Drive file ID'); return; }
  const r = await api('importXlsx', {fileId});
  const s = document.getElementById('importStatus'); s.style.display='block';
  s.textContent = r.ok? 'Import done. Reloading products…' : 'Import failed: ' + (r.error||'');
  if(r.ok){ await loadProducts(); }
}

window.addEventListener('DOMContentLoaded', ()=>{
  const btn = document.getElementById('importXlsxBtn');
  if(btn) btn.addEventListener('click', importXlsx);
});
